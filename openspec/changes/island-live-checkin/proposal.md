...
- 活動結束後**補寄活動訊息**給缺席者（既有通知/email 管道）：這場的題目、參與概況、揭曉牆連結、瓶中信補答入口、下一班船時間。瓶中信不等人回來拿——正在淡出的人不會自己回訪，入口要送到門口。措辭紅線：說「這場發生了什麼」，不說「你錯過了什麼」（邀請，不是催討）。
...
- 缺席補寄：`closes_at` 到點排 BullMQ job（server 既有 queue 基礎建設），對缺席名單經既有通知/email 管道寄送活動記錄與補答入口。
...
- 單場 RSVP——刻意不做：報名單位是「團」（加入 cohort＝報名整季儀式），散客制會讓「缺席」失去定義、瓦解固定 roster 依賴的所有機制。
- 產品內發現入口（遺跡島公開櫥窗、群島導航露出）——撞隱私取捨（牆上是團內真心話），MVP 遺跡島團內可見，第二批再權衡。
