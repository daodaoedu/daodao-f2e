...
「整季」不是長期合約——cohort 天生有 `start_date`/`end_date`（programs→cohorts 本就是一計畫多期結構），**有終點的承諾才是可承受的承諾**，且期末同時是「感覺有進展」的里程碑（最後一場＝畢業場，一季遺跡群＝成果）。彈性由三層設計提供，不靠縮短季長：

**季長不是活動島的參數**：期長由 cohort 自己決定（現實中依 `practice_template.duration_days` 推算、各期不一），場次數＝期長÷週期。活動島只提供給 admin 的**建議區間：週更下 4~8 週**——下界來自節奏成形（約第 4 次重複起「週一晚上＝上島」才變成預設）與缺席容錯（≥5~6 場，缺一場僅 1/6，不觸發掉隊感）；上界來自承諾可視性（終點須在報名當下看得見）、形式新鮮感衰減、與續報數據的取得頻率。定案靠數據：出席衰減曲線定上界、續報率驗承諾重量，前幾期以不同期長試跑校準。

1. **承諾量透明**：同意畫面寫明實際 ask（如「6 週、每週一次、每次 10 分鐘」＝總共一小時；數字隨該期實際設定帶出）。明確的小承諾勝過模糊的大承諾。
2. **缺席零懲罰**（彈性的主體）：無 streak、無出席率、缺席只是可見（火把）；補寄不催討、補答永遠開放。約定是社會性的，不是契約性的——缺席若有懲罰則六週太長，缺席只被想起則六週只是六次邀請。
3. **進出自由**：中途退出（既有 `exited_at`，退出即停通知）、`join_deadline` 前可中途加入、期末續報是主動選擇不自動續約。

續報率＝每期期末天然的留存量測點，是「儀式有沒有用」最誠實的數據。
