...
const MilestonesProgress = ({ milestones = [] }: MilestonesProgressProps) => {
  ...
  const milestoneEndDate = useMemo(() => {
    ...
    return milestones.reduce<Date | null>((d, m) => {
  ...
  return (
    <Panel className="mb-6 flex flex-col gap-3 bg-white md:gap-5 md:py-6">
      <Title title="學習里程碑進度" className="mb-0" />
      {Array.isArray(milestones) && milestones.length > 0 ? (
        <>
          <ProgressBar progress={progressValue} />
          <p className="font-sans text-sm text-basic-300 md:text-base">
        ...
        <>
          ...
          <Panel className="bg-white">
            ...
            {project && !isMarathonProject && (
              <>
                {!isMarathonProject && (
                  <div className="flex flex-col justify-between gap-2 pb-2.5 md:flex-row md:items-center">
                    <div className="flex items-center gap-2">
                      <p>時間：</p>
                      <DateRangePicker
                        value={date}
                        separator={
                          <SwapRightIcon className="h-4 w-4 text-basic-black/25" />
                        }
                        className="-mx-3 p-2"
                        disabled
                      />
                ...
                <div className="flex flex-col justify-between gap-2 pb-2.5 md:flex-row">
                  <div>
                    顯示：
                    {filterItems.map((item) => (
                      <Button
                        ...
                        className="mr-2 rounded-lg px-2.5"
                  ...
                  <Button
                    variant="outline"
                    className="group flex items-center gap-2 rounded-lg px-2.5"
