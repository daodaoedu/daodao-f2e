...
import { COMMON_TAGS, DINING_DESCRIPTIONS, ROLE_IDS } from "./result-detail-constants";
import {
  createPartner,
  createResourceLink,
  createResultDetail,
...
const resultDetailList: IResultDetail[] = [
  createResultDetail({
    ...
    strategies: ["找深度資料當主餐慢慢咀嚼", "整理知識地圖", "刻意留白反思並紀錄觀點"],
    partners: [
      createPartner(
        ROLE_IDS.ORGANIZER,
        "助整理",
      ...
      createPartner(ROLE_IDS.LINKER, "激發靈感", "擅長帶來意想不到的類比與觀點"),
    ],
    learningTraits:
  ...
  createResultDetail({
    ...
    strategies: ["把大任務拆小", "放下高標從最小可行的開始", "選擇有實際產出的學習方式"],
    partners: [
      createPartner(ROLE_IDS.ORGANIZER, "幫我排流程", "提供清晰的任務藍圖與檢查清單"),
      createPartner(ROLE_IDS.CONNECTOR, "幫我測試點子", "能快速組織一個「使用者測試小隊」"),
    ],
    learningTraits:
    ...
    recommendedResourceLinks: [createResourceLink("g0v 零時政府社群", "https://g0v.tw/")],
  }),
  ...
  createResultDetail({
    ...
    strategies: ["先建立整體結構再深入細節", "善用可視化筆記", "歸納之餘也記得要輸出想法"],
    partners: [
  ...
  createResultDetail({
    ...
    strategies: ["思索差異與可能的結合點", "以圖像或故事轉化抽象概念", "與他人交流測試或激發想法"],
    partners: [
      createPartner(
        ROLE_IDS.EXPLORER,
        "提供專業深度",
  ...
  createResultDetail({
    ...
    strategies: ["分享是最好的複習", "找尋或創造與人共學的機會", "保留自我沉澱與對話的時光"],
    partners: [
      createPartner(
        ROLE_IDS.ACTOR,
        "一起實作",
...
export const resultDetailMap = new Map<string, IResultDetail>(
