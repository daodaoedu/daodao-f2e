...
const useEditProfile = () => {
  ...
  const onSubmit = async ({ id, email, type }) => {
    const readyForUpdate = (type === 'update') && (id && email);
    const readyForCreate = (type === 'create');

    if (!readyForUpdate && !readyForCreate) return false;
  ...
  const checkBeforeSubmit = async ({ id, email, type }) => {
    if (validate(userState)) {
      const result = await onSubmit({ id, email, type });
      return result;
    }
    return false;
  };
