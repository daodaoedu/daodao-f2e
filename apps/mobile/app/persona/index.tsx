...
import { ChevronLeft, Lock } from "@tamagui/lucide-icons";
...
export default function PersonaHomeScreen() {
  ...
  const { data: profileData, isLoading: isProfileLoading } = usePersonaProfileMe(undefined, {
    enabled: isAuthenticated,
  });
  const { data: questionsData, isLoading: isQuestionsLoading } = usePersonaQuestions(undefined, {
    enabled: !isAuthenticated,
  });
  ...
  const questions: PersonaQuestionSummary[] = isAuthenticated
    ...
    : (questionsData?.data?.questions ?? []).map((q) => ({ id: q.id, prompt: q.prompt, answer: null }));
    ...
    <SafeAreaView style={{ flex: 1 }} edges={["top"]}>
      <YStack flex={1} backgroundColor="$background">
        ...
        {isLoading ? (
          ...
          <ScrollView flex={1} contentContainerStyle={{ padding: 16, gap: 12 }}>
            ...
            {questions.map((q) => (
              <PersonaQuestionCard key={q.id} question={q} onPress={() => router.push(`/persona/${q.id}`)} />
            ))}
          </ScrollView>
        )}
      </YStack>
