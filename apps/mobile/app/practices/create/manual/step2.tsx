...
import { ChevronLeft, ChevronRight, Check } from '@tamagui/lucide-icons'
...
export default function Step2Screen() {
  ...
  const toggleCustomDay = (day: number) => {
    ...
    if (current.includes(day)) {
      setValue('customDays', current.filter(d => d !== day))
    } else {
                  ...
                  <YStack gap="$2">
                    {frequencyOptions.map(option => (
                      <Button
                        key={option.value}
                        size="$4"
                        backgroundColor={value === option.value ? colors.primary.palest : '$background'}
                        borderWidth={1}
                ...
                <XStack gap="$2" justifyContent="space-between">
                  {weekDays.map(day => (
                    <Button
                      ...
                      width={44}
                      height={44}
                      backgroundColor={customDays.includes(day.value) ? colors.primary.base : '$background'}
                      borderWidth={1}
                      borderColor={customDays.includes(day.value) ? colors.primary.base : '$borderColor'}
                  ...
                  <XStack gap="$2" flexWrap="wrap">
                    {targetDaysOptions.map(days => (
                      ...
                      >
                        <Text
                          fontSize={14}
                          color={value === days ? colors.basic.white : '$color'}
                        >
                          {days} 天
                        </Text>
                      </Button>
