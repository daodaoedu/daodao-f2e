...
import { Animated, Dimensions, Linking, Pressable, View as RNView, StyleSheet } from "react-native";
  ...
  return (
    <RNView style={styles.container}>
      ...
      <Animated.View style={[styles.fixedBannerContainer, { opacity: bannerOpacity }]}>
        <Animated.Image source={bannerImage} style={styles.fixedBanner} resizeMode="cover" />
      </Animated.View>
          ...
          <RNView style={styles.topNav}>
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <RNView style={[styles.topNavIcon, isActive && styles.topNavIconActive]}>
                    <Icon size={24} color={isActive ? colors.logo.cyan : colors.text.dark} />
                  </RNView>
                  <Text fontSize={12} color={colors.text.dark} opacity={isActive ? 1 : 0.5}>
                    {tab.label}
                  </Text>
                </Pressable>
              );
              ...
              <XStack justifyContent="space-between" alignItems="center" marginBottom={24}>
                ...
                <XStack alignItems="center" gap={8}>
                  <Pressable
                    ...
                    style={[styles.checkbox, includeCompleted && styles.checkboxActive]}
                  >
                    {includeCompleted && <Check size={14} color={colors.text.light} />}
                  </Pressable>
              ...
              <YStack gap={10}>
                {displayedPractices.length > 0 ? (
                  displayedPractices.map((practice) => {
                    ...
                    return (
                      <Pressable key={practice.id} onPress={() => handlePracticePress(practice.id)}>
                        <YStack
                          padding={16}
                          borderRadius={8}
                          borderBottomWidth={1}
