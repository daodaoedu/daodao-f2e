...
function InlineAnswerForm({ questionId, questionType, options, onSuccess }: InlineAnswerFormProps) {
  ...
  return (
    <YStack mt="$2" gap="$2">
      <Text color="$gray9" fontSize="$2">點選選項後送出</Text>
      <Button size="$3" onPress={handleSubmit} disabled={submitting}>
        {submitting ? "送出中..." : "送出"}
      </Button>
    </YStack>
  );
...
export function PersonaProfileTab() {
  ...
  const handleAnswerSuccess = async (questionId: number) => {
            ...
            >
              ...
              {isExpanded ? (
                <InlineAnswerForm
                  ...
                  options={q.options}
                  onSuccess={() => handleAnswerSuccess(q.id)}
                />
              ) : (
