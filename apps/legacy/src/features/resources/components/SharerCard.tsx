  ...
  return (
    <div className={cn("rounded-xl", className)}>
      <div className="relative h-[17.625rem] overflow-hidden rounded-t-xl">
        <Image src={userImg} alt="avatar" fill className="object-cover" />
        ...
        {(order === 1 || order === 2) && (
          <div className="absolute left-3 top-3 flex size-9 items-center justify-center rounded-full bg-white">
            <CrownIcon className="size-6" color={order === 1 ? "#FF9526" : "#8F8F8F"} />
          </div>
        )}
      </div>
      ...
      >
        <div className="flex items-center md:h-6">
          ...
          <span className="mx-1 pr-2" style={{ borderRight: "1px solid #DBDBDB" }}>
            分享資源
          </span>
          <span className="font-bold text-primary-base">{sourceCount} 筆</span>
        </div>
        ...
        <div className="flex items-center md:h-6">
          ...
          <span className="font-bold text-primary-base">{reflectionCount} 筆</span>
        </div>
      </div>
    </div>
  );
