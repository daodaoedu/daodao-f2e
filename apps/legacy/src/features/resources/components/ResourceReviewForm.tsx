...
export default function ResourceReviewForm({
  ...
  const { trigger: createResourceReview, isMutating: isCreating } = useCreateResourceReview(
    resourceId,
    {
      onSuccess: () => {
  ...
  const { trigger: updateResourceReview, isMutating: isUpdating } = useUpdateResourceReview(
    resourceId,
    reviewId,
    {
      onSuccess: () => {
            ...
            <Container className="flex items-center justify-end gap-10">
              ...
              <div className="flex shrink-0 gap-3">
                {typeof reviewId !== "number" && (
                  <Button variant="outline" type="button" size="lg" disabled={isSubmitting}>
