...
import type { ResourceFormSchema, ResourceReviewFormSchema } from "@/services/resources";
...
export default function ResourceReviewFields({
  ...
  const form = useFormContext<ResourceReviewFormSchema | ResourceReviewSchema>();
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-6 rounded-lg border border-solid border-basic-100 p-6">
              ...
              render={({ field }) => (
                <FormItem className="basis-full">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <FormControl>
                      <Rating value={field.value ?? 0} onValueChange={field.onChange} max={5} />
                    </FormControl>
                  </div>
                  <FormMessage />
                </FormItem>
              ...
              render={({ field }) => (
                <FormItem className="flex-1 basis-2/5">
                  <div className="mb-2 flex items-center gap-3">
                    <FormLabel className="body-md mb-0 flex-1 text-right">改變思維方式</FormLabel>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 basis-2/5">
                  <div className="mb-2 flex items-center gap-3">
                    <FormLabel className="body-md mb-0 flex-1 text-right">獲得新觀點</FormLabel>
                    <FormControl className="flex-1">
                      <Rating value={field.value ?? 0} onValueChange={field.onChange} max={5} />
                    </FormControl>
                  </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 basis-2/5">
                  <div className="mb-2 flex items-center gap-3">
                    <FormLabel className="body-md mb-0 flex-1 text-right">解決實際問題</FormLabel>
                    <FormControl className="flex-1">
                      <Rating value={field.value ?? 0} onValueChange={field.onChange} max={5} />
                    </FormControl>
                  </div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 basis-2/5">
                  <div className="mb-2 flex items-center gap-3">
                    <FormLabel className="body-md mb-0 flex-1 text-right">達成具體目標</FormLabel>
                    <FormControl className="flex-1">
                      <Rating value={field.value ?? 0} onValueChange={field.onChange} max={5} />
                    </FormControl>
                  </div>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <MarkdownEditor placeholder="最少需填入 20 個字元" maxLength={999} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
                  ...
                  >
                    ...
                    >
                      <FormControl>
                        <Checkbox
                          id={item.value}
                          checked={field.value?.[item.value as keyof typeof field.value] ?? false}
                ...
                <FormItem>
                  ...
                  <MultipleSelector
                    ...
                    value={Object.entries(field.value ?? {})
                      .map(([key, value]) =>
                        resourceUsageOptions.find((option) => option.value === key && value)
                      )
