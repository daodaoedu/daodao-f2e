...
import ResponsiveModal, { type ResponsiveModalProps } from "@/shared/ui/responsive-modal";

export interface DialogContentProps {
  close: () => void;
}

export type RenderDialogContent = (props: DialogContentProps) => React.ReactNode;

interface DialogProps
  extends Omit<ResponsiveModalProps, "children" | "open" | "onClose" | "footer"> {
...
export const DialogProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const footer = disableFooter ? null : (
    <div className={cn("w-full", footerClassName)}>
      ...
      {footerDescription && <p className="text-center text-basic-400">{footerDescription}</p>}
    </div>
  );
    ...
    <DialogContext.Provider value={value}>
      ...
      >
        <div className={cn("p-4 pb-0 text-center", className)}>
          {typeof content === "function" ? content({ close: handleCloseDialog }) : content}
        </div>
      </ResponsiveModal>
