...
const getAvailableOptions = (
  ...
  return allOptions.filter(
    (option) => !selectedPlatforms.includes(option.value) || option.value === currentPlatform
  );
};
...
>({
  ...
  const watchedValue = useWatch({ control, name }) as Record<string, string> | undefined;
  ...
  // 更新聯絡項目
  const handleUpdateItem = (id: string, fieldType: "platform" | "value", newValue: string) => {
  ...
  return (
    <div className={cn("space-y-3", className)}>
      {items.map((item) => {
        const availableOptions = getAvailableOptions(options, selectedPlatforms, item.platform);
          ...
          <div key={item.id} className="flex items-center gap-2">
            ...
            <div className="flex-1">
              <Selector
                ...
                value={item.platform}
                onValueChange={(value) => handleUpdateItem(item.id, "platform", value || "")}
              />
            </div>
