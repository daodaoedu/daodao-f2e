...
function TagEditor({ name, helperText, control, value = [], tagOptions }) {
  ...
  const handleKeyDown = (e) => {
    switch (e.keyCode) {
      ...
      case 40: {
        if (!hasTagOptions) return;
        tagOptionsFocusIndex.current = Math.min(
          filteredTagOptions.length - 1,
      ...
      {hasTagOptions && (
        ...
        >
          {filteredTagOptions.map((text) => (
            <Item key={text} text={text} onClick={handleAddTag} />
          ))}
        </ul>
      )}
      ...
      {!!value.length && (
        <ul className="flex flex-wrap gap-2">
          {value.map((tag) => (
            <li
              key={tag}
              className="inline-flex items-center gap-1 rounded-md bg-primary-lightest px-2 py-1 text-sm"
            >
              <span>{tag}</span>
              <button
                type="button"
