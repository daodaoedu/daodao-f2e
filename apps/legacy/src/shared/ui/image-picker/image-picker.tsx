...
const fetchRandomImage = async ([width, height, random]: [number, number, number]) => {
  const response = await fetch(`https://picsum.photos/${width}/${height}?random=${random}`);
...
export const ImagePicker = forwardRef<HTMLButtonElement, ImagePickerProps>(
  (
    ...
    const { data, error, isValidating } = useSWR<string>(
      tab === "random" ? [Math.floor(height * ratio) * 2, Math.floor(height) * 2, random] : null,
      fetchRandomImage,
      {
        errorRetryCount: 2,
    ...
    // 使用多檔案上傳 hook
    const { previewUrls, hasFiles, inputRef, handleInputChange, triggerFileSelect } =
    ...
    return (
      <>
        <AspectRatio ratio={ratio}>
          ...
          >
            {hasImage && displayUrl && (
              <Image src={displayUrl} alt="upload" className="object-cover" fill />
            )}
            <span
              className={cn(
          ...
          >
            ...
            <TabsContent value={TabEnum.Upload} className="md:p-6 lg:p-6">
              <div>
                ...
                >
                  ...
                  <Button type="button" variant="outline" size="lg" onClick={triggerFileSelect}>
                    選取檔案
                  </Button>
                </div>
            ...
            <TabsContent value={TabEnum.Random} className="md:p-6 lg:p-6">
              <div className="mb-4 flex justify-center">
                ...
                {!isValidating && error && (
                  <Skeleton className="bg-alert/10" style={{ width: `${height * ratio}px` }}>
                    <AspectRatio
                      ratio={ratio}
                ...
                {!isValidating && !error && data && (
                  <Image src={data} alt="random" width={height * ratio} height={height} />
                )}
              </div>
              <div className="flex justify-center">
                <Button type="button" variant="outline" size="lg" onClick={dispatchRandom}>
                  <Dice5Icon size={15} />
                  隨機生成
                </Button>
              </div>
