...
export const ImageDialog = ({ open, onClose, onSave, initialData }: ImageDialogProps) => {
  ...
  return (
    <ResponsiveModal open={open} onClose={reset} title="上傳圖片" footer={footer}>
      <div className="flex flex-col gap-4">
        <div className="grid gap-2">
          <Label htmlFor="src">從網址新增圖片：</Label>
          <Input
            type="text"
            id="src"
            value={imageData.src}
        ...
        <div className="grid gap-2">
          ...
          <Input type="text" id="title" value={imageData.title} onChange={handleChange("title")} />
        </div>
      </div>
    </ResponsiveModal>
  );
