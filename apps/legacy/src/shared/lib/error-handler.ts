 ...
 * @returns 錯誤訊息字串
 */
export const getErrorMessage = (error: unknown, defaultMessage = "發生未知錯誤"): string => {
  const detail = (error as ApiError)?.data?.details?.[0];

  return (
    (hasDetailMessage(detail) && detail.message) || (error as Error)?.message || defaultMessage
  );
};
...
export const handleError = (error: unknown, options: ErrorHandlerOptions = {}) => {
  const { defaultMessage = "操作失敗，請稍後再試", showToast = true, logError = true } = options;
