...
export class EventBuilder extends JsonLdBuilder<Event> {
  ...
  setStartDate(startDate: string | Date): this {
    ...
    this.data.startDate = startDate instanceof Date ? startDate.toISOString() : startDate;
    return this;
  }

  setEndDate(endDate: string | Date): this {
    if (!endDate) return this;
    this.data.endDate = endDate instanceof Date ? endDate.toISOString() : endDate;
    return this;
  }
  ...
  setLocation(
    ...
    this.data.location =
      typeof location === "string" ? { "@type": "Place", name: location } : location;
    return this;
  }
