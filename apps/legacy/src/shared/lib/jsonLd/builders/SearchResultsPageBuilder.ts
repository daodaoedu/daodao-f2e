import type { SearchResultsPage, Thing } from "schema-dts";
import { type BreadcrumbItem, BreadcrumbListBuilder } from "./BreadcrumbListBuilder";
...
export class SearchResultsPageBuilder extends JsonLdBuilder<SearchResultsPage> {
  ...
  private getKeywordsElement(): string[] {
    const keywords: string[] = Array.isArray(this.data.keywords) ? this.data.keywords : [];
  ...
  setLastReviewed(lastReviewed: string | Date): this {
    ...
    this.data.lastReviewed =
      lastReviewed instanceof Date ? lastReviewed.toISOString() : lastReviewed;
    return this;
  }

  setBreadcrumb(breadcrumb: BreadcrumbItem[]): this {
    this.data.breadcrumb = new BreadcrumbListBuilder().setItems(breadcrumb).buildWithoutContext();
    return this;
  }
