...
interface UseDraggableContainerProps<T> {
   ...
   * @returns 更新後的項目
   */
  updateItem: (currentItem: T, overItem: T, oldIndex: number, newIndex: number) => T;
  /**
   * 拖拽結束後的回調函數
   * @param updatedItem 更新後的項目
   * @returns 如果返回 false，則不更新內部狀態
   */
  onReorder?: (updatedItem: T) => undefined | Promise<boolean>;
}
...
export const useDraggableContainer = <T>({
    ...
    async (event: DragEndEvent) => {
      ...
      if (over && active.id !== over.id) {
        ...
        if (oldIndex !== -1 && newIndex !== -1) {
          ...
          // 使用提供的函數來更新項目位置
          const updatedItem = updateItem(currentItem, overItem, oldIndex, newIndex);
  ...
  const overItem = overId ? items.find((item) => getItemId(item) === overId) : null;
