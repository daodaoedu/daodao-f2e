...
const env = getEnv();
export const V1_BASE_URL = env.apiUrl || "https://api.daoedu.tw";
...
const validValueSchema = z.union([z.string().min(1), z.number(), z.boolean(), z.instanceof(Blob)]);
...
const serialize =
  <T extends URLSearchParams | FormData>(formattedData: T, keys: string[] = []) =>
  (source: unknown) => {
...
export const fetcher = <R = void>(params: FetcherParams): Promise<R> => {
  ...
  const source = args.filter(isRecord).reduce((acc, curr) => ({ ...acc, ...curr }), {});
  return http<R>({ pathname, source });
};

export const fetcherV1 = <R = void>(params: FetcherParams, options: RequestInit = {}): Promise<R> =>
