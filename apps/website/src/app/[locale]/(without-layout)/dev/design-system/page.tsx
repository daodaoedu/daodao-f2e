...
const primaryColors = [
  ...
  { name: "Primary Lightest",token: "primary.lightest",hex: "#B3E8E6", cssVar: "--primary-lightest" },
  { name: "Primary Lighter", token: "primary.lighter", hex: "#66D4CF", cssVar: "--primary-lighter" },
  { name: "Primary Base",    token: "primary.base",    hex: "#16B9B3", cssVar: "--primary-base", isMain: true },
  ...
  { name: "Mascot Bright Blue", hex: "#5CC5E8", cssVar: "--mascot-bright-blue", token: "mascot.brightBlue" },
];
const grayColors = [
...
const fontFamilies = [
  ...
  { name: "Anonymous Pro",  value: '"Anonymous Pro", monospace',       token: "CSS: .anonymous-pro", mono: true },
];
...
const spacingProject = [
  {k:"0",   px:0},  {k:"0.5",px:2},  {k:"1",  px:4},  {k:"1.5",px:6},
  {k:"2",   px:8},  {k:"2.5",px:10}, {k:"3",  px:12}, {k:"3.5",px:14},
  {k:"4",   px:16}, {k:"5",  px:20}, {k:"6",  px:24}, {k:"7",  px:28},
  {k:"8",   px:32}, {k:"9",  px:36}, {k:"10", px:40}, {k:"11", px:44},
  {k:"12",  px:48}, {k:"14", px:56}, {k:"16", px:64}, {k:"20", px:80},
  {k:"24",  px:96}, {k:"28", px:112},{k:"32", px:128},
];
const spacingTokensOnly = [
  {k:"36",px:144},{k:"40",px:160},{k:"44",px:176},{k:"48",px:192},
  {k:"52",px:208},{k:"56",px:224},{k:"60",px:240},{k:"64",px:256},
  {k:"72",px:288},{k:"80",px:320},{k:"96",px:384},
];
...
const breakpoints = [
  ...
  { name: "lg",  px: 1024 }, { name: "xl",  px: 1280 },
  { name: "2xl", px: 1536 },
];
const containers = [
  { name: "xs",  px: 320  }, { name: "sm",  px: 384  }, { name: "md",  px: 448 },
  { name: "lg",  px: 512  }, { name: "xl",  px: 576  }, { name: "2xl", px: 672 },
  { name: "3xl", px: 768  }, { name: "4xl", px: 896  }, { name: "5xl", px: 1024},
  { name: "6xl", px: 1152 }, { name: "7xl", px: 1280 },
];
...
const blurs        = [
  { name: "none",    px: 0  }, { name: "sm",      px: 4  },
  { name: "DEFAULT", px: 8  }, { name: "md",      px: 12 },
  { name: "lg",      px: 16 }, { name: "xl",      px: 24 },
  { name: "2xl",     px: 40 }, { name: "3xl",     px: 64 },
];
const borderWidths = [
  { name: "0",       px: 0 }, { name: "DEFAULT", px: 1 },
  { name: "2",       px: 2 }, { name: "4",       px: 4 },
  { name: "8",       px: 8 },
];
...
function TokenBadge({ children, dim }: { children: string; dim?: boolean }) {
  return (
    <code className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${dim ? "bg-orange-50 text-orange-400" : "bg-gray-100 text-gray-500"}`}>
      {children}
    </code>
  );
}
...
function ColorSwatch({ hex, name, sub, badge }: { hex:string; name:string; sub?:string; badge?:string }) {
  const light = isLight(hex);
  return (
    <div className="rounded-xl overflow-hidden shadow-sm" style={{ border: hex === "#FFFFFF" || hex === "#F5FFFD" || hex === "#F4F6F6" || hex === "#FAFAFA" ? "1px solid #d1d5db" : "1px solid transparent" }}>
      <div className="h-14 w-full flex items-end px-2.5 pb-1.5 relative" style={{ backgroundColor: hex }}>
        {badge && (
          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full absolute top-1.5 right-1.5"
            style={{ backgroundColor: light ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.25)", color: light ? "#1A2B3C" : "#FFF" }}>
            {badge}
          </span>
        )}
      </div>
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-gray-100 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 flex items-center gap-2 h-12 overflow-x-auto scrollbar-hide">
          <div className="flex items-center gap-2 mr-4 shrink-0">
            ...
            <span className="text-xs font-bold text-[#16B9B3] uppercase tracking-widest">DaoDao DS</span>
          </div>
          {NAV.map(n => (
      ...
      <div className="max-w-5xl mx-auto px-4 py-12 space-y-20">
        ...
        <header className="space-y-2">
          ...
          <p className="text-sm text-gray-400 max-w-xl">
            設計端與開發端共同確認的 Token 手冊。
            來源：<code className="text-xs bg-gray-200 px-1 rounded">packages/design-tokens/src/</code>
            　對照：<code className="text-xs bg-orange-100 text-orange-500 px-1 rounded">tokens.json</code>
          </p>
          <div className="flex gap-3 pt-1 flex-wrap">
            <span className="text-xs flex items-center gap-1.5 bg-white border border-gray-200 rounded-full px-3 py-1">
              <span className="w-2 h-2 rounded-full bg-[#16B9B3] inline-block" />專案 Token（已實作）
            </span>
            <span className="text-xs flex items-center gap-1.5 bg-orange-50 border border-orange-200 rounded-full px-3 py-1">
              <span className="w-2 h-2 rounded-full bg-orange-400 inline-block" />⚠ tokens.json only（待確認是否補入）
            </span>
          </div>
        </header>
        ...
        <section id="color">
          ...
          <SectionDesc>來源：packages/design-tokens/src/colors.ts ＋ Figma Design System Color 頁面</SectionDesc>

          <SubTitle>主色 Primary Scale</SubTitle>
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-10">
            {primaryColors.map(c => (
              <div key={c.hex}>
                <ColorSwatch hex={c.hex} name={c.name} sub={c.cssVar} badge={c.isMain ? "Base" : undefined} />
                {c.token && <div className="mt-1.5"><TokenBadge>{c.token}</TokenBadge></div>}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-10">
            {grayColors.map(c => (
              <div key={c.token}>
                ...
                <div className="mt-1.5"><TokenBadge>{c.token}</TokenBadge></div>
              </div>
            ))}
          </div>
        ...
        <section id="typography">
          ...
          <SectionDesc>來源：packages/design-tokens/src/typography.ts　⚠ 灰底 = tokens.json 有但專案缺失</SectionDesc>

          {/* Font Families */}
          <SubTitle>字型 Font Families</SubTitle>
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-10">
            {fontFamilies.map((f, i) => (
              <div key={f.name} className={`flex items-center gap-4 px-6 py-4 ${i < fontFamilies.length-1 ? "border-b border-gray-100" : ""}`}>
                ...
                <code className="text-[10px] text-gray-300 hidden sm:block shrink-0">{f.value}</code>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-10">
            {[...fontSizes].reverse().map((s, i, arr) => (
              <div key={s.name} className={`flex items-center gap-4 px-6 py-2 ${!s.inProject ? "bg-orange-50/60" : ""} ${i < arr.length-1 ? "border-b border-gray-50" : ""}`}>
                <div className="w-32 shrink-0">
                  <div className="flex items-center gap-1.5">
                    <TokenBadge dim={!s.inProject}>{s.name}</TokenBadge>
                    {!s.inProject && <MissingBadge />}
                  </div>
                  <div className="text-[10px] text-gray-400 mt-1">{s.px}px / {s.rem}</div>
                </div>
                <div className="text-gray-700 flex-1 truncate" style={{ fontSize: Math.min(s.px, 72), lineHeight: 1.3 }}>
                  Dao dao 島島阿學
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mb-10">
            {fontWeights.map(w => (
              <div key={w.token} className={`rounded-xl p-4 shadow-sm ${w.inProject ? "bg-white" : "bg-orange-50 border border-orange-100"}`}>
                <div className="text-xl text-gray-700 mb-2" style={{ fontWeight: w.value }}>Aa 漢字</div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-10">
            {lineHeights.map(lh => (
              <div key={lh.token} className="bg-white rounded-xl p-4 shadow-sm">
                <div className="text-sm text-gray-700 mb-3 border-l-2 border-[#16B9B3] pl-3" style={{ lineHeight: lh.ratio }}>
                  DaoDao 道道學習平台<br />讓學習成為習慣的好夥伴
                </div>
          ...
          {/* Letter Spacing */}
          <SubTitle>字距 Letter Spacing <span className="ml-2 text-[10px] text-orange-400 font-normal normal-case">⚠ tokens.json 有，專案缺失</span></SubTitle>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-2">
            {letterSpacings.map(ls => (
              <div key={ls.token} className="bg-orange-50 border border-orange-100 rounded-xl p-4 shadow-sm">
        ...
        <section id="radius">
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-8">
            ...
            {radiusComparison.map((r, i) => (
              <div key={r.name} className={`grid grid-cols-4 items-center px-6 py-3 ${i < radiusComparison.length-1 ? "border-b border-gray-50" : ""}`}>
                <code className="text-xs text-gray-600 font-mono">{r.name}</code>
                {/* Project swatch */}
                <div className="flex items-center justify-center gap-2">
                  <div className="w-8 h-8 bg-[#16B9B3]/20 border-2 border-[#16B9B3]/40"
                    style={{ borderRadius: r.projectPx >= 9999 ? "50%" : r.projectPx }} />
                  <span className="text-xs text-gray-500">{r.projectPx >= 9999 ? "full" : `${r.projectPx}px`}</span>
                </div>
                {/* tokens.json swatch */}
                <div className="flex items-center justify-center gap-2">
                  <div className="w-8 h-8 bg-orange-200/50 border-2 border-orange-300/50"
                    style={{ borderRadius: r.tokensPx >= 999 ? "50%" : r.tokensPx }} />
                  <span className="text-xs text-orange-400">{r.tokensPx >= 999 ? "full" : `${r.tokensPx}px`}</span>
                </div>
                {/* diff */}
                <div className="text-center">
                  {r.projectPx === r.tokensPx || (r.projectPx >= 9999 && r.tokensPx >= 999)
                    ...
                    : <span className="text-xs text-orange-400 font-medium">
                        {r.projectPx >= 9999 ? "—" : `${r.projectPx - r.tokensPx > 0 ? "+" : ""}${r.projectPx - r.tokensPx}px`}
                      </span>}
                </div>
              </div>
            ))}
        ...
        <section id="spacing">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {spacingProject.map(s => (
              <div key={s.k} className="flex flex-col items-center gap-1 bg-white rounded-lg p-2 shadow-sm min-w-[52px]">
                <div className="bg-[#16B9B3]/30 border border-[#16B9B3]/40 rounded"
          ...
          <SubTitle>tokens.json 額外擴展 <span className="ml-2 text-[10px] text-orange-400 font-normal normal-case">⚠ 專案缺失</span></SubTitle>
          <div className="flex flex-wrap gap-2">
            {spacingTokensOnly.map(s => (
              <div key={s.k} className="flex flex-col items-center gap-1 bg-orange-50 border border-orange-100 rounded-lg p-2 shadow-sm min-w-[52px]">
                <div className="bg-orange-200/50 border border-orange-300/40 rounded" style={{ width: 64, height: 16 }} />
        ...
        <section id="layout">
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-8">
            {breakpoints.map((b, i) => (
              <div key={b.name} className={`flex items-center gap-4 px-6 py-3 ${i < breakpoints.length-1 ? "border-b border-gray-50" : ""}`}>
                <code className="w-12 text-sm font-mono text-gray-600 shrink-0">{b.name}</code>
                <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-[#16B9B3]/40 rounded-full" style={{ width: `${(b.px / 1536) * 100}%` }} />
                </div>
                <span className="text-sm font-mono text-gray-500 shrink-0 w-16 text-right">{b.px}px</span>
                <TokenBadge dim>{`layout.breakpoint${b.name.charAt(0).toUpperCase()}${b.name.slice(1)}`}</TokenBadge>
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            {containers.map((c, i) => (
              <div key={c.name} className={`flex items-center gap-4 px-6 py-3 ${i < containers.length-1 ? "border-b border-gray-50" : ""}`}>
                <code className="w-12 text-sm font-mono text-gray-600 shrink-0">{c.name}</code>
                <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-[#66D4CF]/50 rounded-full" style={{ width: `${(c.px / 1280) * 100}%` }} />
                </div>
                <span className="text-sm font-mono text-gray-500 shrink-0 w-16 text-right">{c.px}px</span>
                <TokenBadge dim>{`layout.container${c.name.charAt(0).toUpperCase()}${c.name.slice(1)}`}</TokenBadge>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="shadow">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            {shadows.map(s => (
              <div key={s.name} className="bg-white rounded-xl p-4 flex flex-col items-center gap-3">
                <div className="w-14 h-14 rounded-xl bg-white"
                  style={{ boxShadow: s.css === "none" ? "none" : s.css, border: "1px solid #f3f4f6" }} />
                <div className="text-center">
                  <code className="text-xs font-mono text-gray-600">{s.name}</code>
                  <div className="mt-1"><TokenBadge dim>{`shadow.${s.name}`}</TokenBadge></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <section id="other">
          ...
          <div className="flex flex-wrap gap-2 mb-10">
            {opacities.map(o => (
              <div key={o} className="flex flex-col items-center gap-1.5 bg-white rounded-xl p-3 shadow-sm min-w-[52px]">
                <div className="w-8 h-8 rounded-lg border border-gray-200 bg-[#16B9B3]" style={{ opacity: o / 100 }} />
          ...
          <div className="flex flex-wrap gap-3 mb-10">
            {blurs.map(b => (
              <div key={b.name} className="flex flex-col items-center gap-2 bg-white rounded-xl p-3 shadow-sm min-w-[72px]">
                <div className="w-12 h-12 rounded-lg relative overflow-hidden">
                  ...
                  <div className="absolute inset-0" style={{ backdropFilter: `blur(${b.px}px)`, WebkitBackdropFilter: `blur(${b.px}px)` }} />
                </div>
                <code className="text-[11px] font-mono text-orange-500">{b.name}</code>
                <span className="text-[9px] text-gray-400">{b.px}px</span>
                <TokenBadge dim>{`blur${b.name.charAt(0).toUpperCase()+b.name.slice(1)}`}</TokenBadge>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            {borderWidths.map(bw => (
              <div key={bw.name} className="flex flex-col items-center gap-2 bg-white rounded-xl p-4 shadow-sm min-w-[80px]">
                <div className="w-12 h-12 rounded-lg border-[#16B9B3]" style={{ borderWidth: bw.px, borderStyle: "solid", borderColor: "#16B9B3" }} />
        ...
        <footer className="text-center text-xs text-gray-300 pb-4 border-t border-gray-200 pt-6 space-y-1">
          ...
          <div>Source A: <code>packages/design-tokens/src/</code>　Source B: <code>tokens.json</code>　Figma: DaoDao-Design-System</div>
        </footer>
      </div>
    </div>
  );
