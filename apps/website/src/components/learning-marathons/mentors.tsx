...
const MentorCard = ({ mentor, children, className, style, onClick }: MentorCardProps) => (
  <button
    key={mentor?.name}
    type="button"
    className={cn(
  ...
  >
    ...
    <div className="absolute inset-0 bg-linear-to-b from-transparent via-transparent to-primary-base/80" />
    {children}
  </button>
);
    ...
    <>
                  ...
                  <div className="absolute inset-x-0 bottom-0 pt-4">
                    <div className="flex gap-2 px-3">
                      {mentor.tags.slice(0, 1).map((tag, index) => (
                        <Tag key={tag} text={tag} className={index === 0 ? "shrink-0" : ""} />
                      ))}
                    </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="relative max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg bg-white p-6 shadow-lg">
            <button
              type="button"
