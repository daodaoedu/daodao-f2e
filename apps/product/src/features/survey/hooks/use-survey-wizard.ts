...
type WizardAction =
  ...
  | { type: "UPDATE_QUESTION"; index: number; question: Partial<CreateSurveyInput["questions"][number]> }
...
function reducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    ...
    case "UPDATE_CONFIG":
      return { ...state, survey: { ...state.survey, config: { ...state.survey.config, ...action.config } } }
    case "RESTORE_DRAFT":
      return action.draft
    case "RESET":
...
export function useSurveyWizard(initialDraftKey?: string) {
  ...
  return {
    ...
    setDescription: useCallback((d: string) => dispatch({ type: "SET_DESCRIPTION", description: d }), []),
    setPurpose: useCallback((p: string) => dispatch({ type: "SET_PURPOSE", purpose: p }), []),
    setTone: useCallback((t: AIGenerateInput["tone"]) => dispatch({ type: "SET_TONE", tone: t }), []),
    setAudience: useCallback((a: string) => dispatch({ type: "SET_AUDIENCE", audience: a }), []),
    setQuestionCount: useCallback((n: number) => dispatch({ type: "SET_QUESTION_COUNT", count: n }), []),
    setGeneratedQuestions: useCallback((q: AIGeneratedQuestion[]) => dispatch({ type: "SET_GENERATED_QUESTIONS", questions: q }), []),
    setIsGenerating: useCallback((v: boolean) => dispatch({ type: "SET_IS_GENERATING", isGenerating: v }), []),
    addQuestion: useCallback((q: CreateSurveyInput["questions"][number]) => dispatch({ type: "ADD_QUESTION", question: q }), []),
    updateQuestion: useCallback((i: number, q: Partial<CreateSurveyInput["questions"][number]>) => dispatch({ type: "UPDATE_QUESTION", index: i, question: q }), []),
    removeQuestion: useCallback((i: number) => dispatch({ type: "REMOVE_QUESTION", index: i }), []),
    reorderQuestions: useCallback((from: number, to: number) => dispatch({ type: "REORDER_QUESTIONS", fromIndex: from, toIndex: to }), []),
    updateConfig: useCallback((c: Partial<CreateSurveyInput["config"]>) => dispatch({ type: "UPDATE_CONFIG", config: c }), []),
    saveDraft: useCallback(() => {
      try { localStorage.setItem(state.draftKey, JSON.stringify(state)) } catch { /* ignore */ }
    }, [state]),
