...
export function SurveyResponsePage() {
  ...
  const isQuestionVisible = useCallback((question: typeof questions[number]): boolean => {
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-4">
      ...
      <Button variant="outline" onClick={() => router.push("/")}>回到首頁</Button>
    </div>
  )
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-primary/10 border-b">
        <div className="max-w-2xl mx-auto px-6 py-12">
          ...
          {survey.description && (
            <p className="mt-3 text-muted-foreground text-sm whitespace-pre-wrap">{survey.description}</p>
          )}
        </div>
      </div>
      ...
      <main className="max-w-2xl mx-auto px-6 py-10 space-y-8">
        {questions.map((question, index) => (
          ...
          >
            <div className="mb-4 flex items-start gap-2">
              <span className="text-xs font-mono text-muted-foreground mt-0.5 shrink-0">{index + 1}.</span>
        ...
        <div className="pt-2 pb-16">
          ...
          >
            {submitting ? (
              <><Loader2 className="h-4 w-4 animate-spin mr-2" />提交中...</>
            ) : (
