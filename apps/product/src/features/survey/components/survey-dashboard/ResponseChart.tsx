...
export function ResponseChart({ stat }: ResponseChartProps) {
  ...
  if (questionType === 'yesno') {
    ...
    const pieData = distribution && distribution.length > 0
      ? distribution.map((item, i) => ({ name: item.label, value: item.count, fill: i === 0 ? PRIMARY_COLOR : SECONDARY_COLOR }))
      : [
  ...
  if ((questionType === 'rating' || questionType === 'scale') && !distribution?.length && averageScore != null) {
    return (
      <div className="flex flex-col items-center justify-center h-[200px] gap-2">
        <p className="text-xs text-muted-foreground">平均分</p>
        <p className="text-4xl font-bold" style={{ color: PRIMARY_COLOR }}>{averageScore.toFixed(1)}</p>
      </div>
    )
  }

  if (distribution && distribution.length > 0) {
    const barData = distribution.map((item) => ({ name: item.label, 回應數: item.count, 百分比: item.percentage }))
