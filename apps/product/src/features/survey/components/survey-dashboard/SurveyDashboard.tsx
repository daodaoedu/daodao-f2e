  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        ...
        <Badge variant="outline-ghost" className="w-fit text-xs">
          {stat.questionType}
        </Badge>
      </CardHeader>
      <CardContent className="space-y-2">
        ...
        {averageScore != null && (
          <p className="text-sm text-muted-foreground">
            平均分：<span className="font-medium text-foreground">{averageScore.toFixed(1)}</span>
          </p>
        )}
        {responseCount != null && (
          <p className="text-xs text-muted-foreground">
            回應數：{responseCount}　略過率：{((skipRate ?? 0) * 100).toFixed(0)}%
          </p>
        )}
      </CardContent>
    </Card>
...
export function SurveyDashboard({ surveyId }: { surveyId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Drop-off */}
      {(analytics.trendData as { dropOff?: Array<{ last_position: number; count: number }> })
        ?.dropOff?.length
        ? (() => {
            const dropOff = (
              analytics.trendData as { dropOff: Array<{ last_position: number; count: number }> }
