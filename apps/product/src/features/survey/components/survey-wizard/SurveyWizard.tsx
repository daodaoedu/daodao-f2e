    ...
    <div className="flex min-h-screen flex-col items-center justify-start bg-background py-8 px-4">
      <div className="w-full max-w-2xl">
        ...
        <Card>
          ...
          <CardContent>{StepComponent && <StepComponent wizard={wizard} />}</CardContent>
          <CardFooter className="flex justify-between">
            ...
            <div className="flex gap-2">
              {currentStep !== "publish" && (
                <Button variant="outline" onClick={wizard.saveDraft}>
                  儲存草稿
                </Button>
              )}
              {currentStep !== "publish" && <Button onClick={wizard.nextStep}>下一步</Button>}
            </div>
          </CardFooter>
        </Card>
      </div>
