import type { AIGenerateInput, CreateSurveyInput, SubmitResponseInput } from "../schema";
import type {
  AIGenerateResponse,
  QuestionType,
  Survey,
...
export async function createSurvey(input: CreateSurveyInput): Promise<SurveyWithQuestions> {
  ...
  const localIdToPosition = new Map(
    (input.questions as Array<(typeof input.questions)[number] & { id?: string }>).map((q, i) => [
      q.id ?? String(i),
      i,
    ])
  );
...
export async function updateSurvey(
  id: string,
  input: Partial<CreateSurveyInput> & { coverUrl?: string | null }
): Promise<Survey> {
...
export async function submitResponse(input: SubmitResponseInput): Promise<{ response_id: number }> {
  const answers = input.answers.map((a) => {
    ...
    if (Array.isArray(v))
      return { question_id: Number(a.questionId), selected_option_ids: v.map(Number) };
...
export async function trackPartial(
  shareId: string,
  sessionKey: string,
  lastPosition: number
): Promise<void> {
