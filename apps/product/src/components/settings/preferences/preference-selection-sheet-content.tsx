      ...
      <div className="flex-1 overflow-y-auto px-4 pt-4 pb-16 space-y-8">
            ...
            <div className="flex flex-col gap-2.5">
              {unselectedOptions.map((option) => (
                <Button
                  ...
                  aria-label={t("add_aria_label", { name: option.name })}
                >
        ...
        <div className="space-y-3">
          ...
          {selectedOptions.length > 0 ? (
            <div className="flex flex-col gap-2.5 p-3 bg-light-blue border border-blue rounded-lg">
              {selectedOptions.map((option) => (
                <Button
                  ...
                  aria-label={t("remove_aria_label", { name: option.name })}
                >
            ...
            <div className="flex flex-col gap-2.5 p-3 bg-light-blue border border-blue rounded-lg">
              <p className="text-sm text-text-dark text-center py-4">{t("no_options_selected")}</p>
            </div>
          )}
          {!hasSelection && <p className="text-sm text-red">{t("select_at_least_one")}</p>}
        </div>
      </div>
