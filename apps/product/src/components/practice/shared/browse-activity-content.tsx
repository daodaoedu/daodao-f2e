...
function FollowerRow({
  ...
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      >
        {isFollowing ? t("action_unfollow") : t("browse_follow")}
      </Button>
    </div>
  );
}
    ...
    <div className="flex flex-col">
      <div className="flex border-b border-[#E4EAE9] mx-4">
        {(["data", "echo"] as const).map((currentTab) => (
          ...
          >
            {currentTab === "data" ? t("browse_tab_data") : t("browse_tab_echo")}
            {tab === currentTab && (
      ...
      {tab === "data" && (
        <div className="flex flex-col divide-y divide-[#E4EAE9] px-4 mt-2">
          {[
            { icon: <TelescopeSvg className="size-5" />, label: t("browse_stat_views"), count: viewCount },
            { icon: <DialogOutlineSvg className="size-5" />, label: t("tab_comments"), count: commentCount },
            {
              icon: <ChartColumnIncreasingSvg className="size-5" />,
              label: t("browse_tab_echo"),
        ...
        <div className="flex flex-col">
          {followers.length > 0 && (
            <div className="flex overflow-x-auto border-b border-[#E4EAE9] mx-4 mt-1">
              ...
              >
                {t("browse_all_count", { count: followers.length })}
                {reactionFilter === "all" && (
                  <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-logo-cyan rounded-full" />
                )}
              </Button>
          ...
          <div className="flex flex-col gap-1 px-4 mt-2">
            {filteredFollowers.length > 0 ? (
              ...
              <div className="py-6 text-center text-sm text-[#9FB5B8]">{t("browse_no_activity")}</div>
            )}
          </div>
        </div>
      )}
