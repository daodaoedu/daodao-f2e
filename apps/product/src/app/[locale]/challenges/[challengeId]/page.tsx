"use client";

import { useParams } from "@daodao/i18n/navigation";
import { ChallengeDetailPage } from "@/components/challenges";
import { getChallengeById } from "@/components/challenges/mock-data";
import { PageHeader } from "@/components/layout";

export default function ChallengeDetailRoute() {
  const params = useParams<{ challengeId: string }>();
  const challenge = getChallengeById(params.challengeId);

  if (!challenge) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <p className="text-sm text-text-secondary">找不到這個挑戰</p>
      </div>
    );
  }

  return (
    <div className="relative w-screen min-h-screen z-10 overflow-hidden overflow-y-auto bg-white">
      <PageHeader leftAction="back" leftLabel="" title={challenge.title} />
      <main className="max-w-[640px] mx-auto pb-10">
        <ChallengeDetailPage challenge={challenge} />
      </main>
    </div>
  );
}
